...
class CalibratedRegressor():
    ...
    def fit(self, X, y) -> 'CalibratedRegressor':
        ...
        # create data
        cdf_true = [
            len([1 for cdf_i in cdf_pred if cdf_i <= cdf_lvl])
            / float(len(cdf_pred))
    ...
    def predict(
        self, X: pd.DataFrame, days_idx: Optional[pd.MultiIndex] = None
    ) -> pd.DataFrame:
        ...
        l_out: List[pd.DataFrame] = []
        for qi in sorted_q:
            q_actual = self.true2pred_.predict([qi])[0]
            l_out += [self.model.predict_quantile(X, q=q_actual).y_pred.values]
