...
class EnsembleDNNForecaster(BaseEstimator, RegressorMixin):
  ...
  def predict(self, X):
    ...
    y_all = []
    var_all = []
    for regressor_ in self.regressors_:
      y, variance = regressor_.predict(X)
      y_all.append(y)
      var_all.append(variance)
    y_all = np.stack(y_all, axis=1)
    var_all = np.stack(var_all, axis=1)
    
    y = np.mean(y_all, axis=1)
    variance = np.mean(var_all + y_all**2, axis=1) - y**2
  ...
  def predict_density(self, X, y):
    ...
    var_all = []
    for regressor_ in self.regressors_:
      y0, variance = regressor_.predict(X)
      y_all.append(y0)
      var_all.append(variance)
    y_all = np.stack(y_all,axis=1)
    var_all = np.stack(var_all,axis=1)
    
    y0 = np.mean(y_all,axis=1)
    variance = np.mean(var_all + y_all**2,axis=1) - y0**2
    mean = y0
...
class BayesianDNNRegressor(BaseEstimator, RegressorMixin):
  ...
  def predict(self, X):
    ...
    #   result[sample] = self.regressor_(X).flatten()
